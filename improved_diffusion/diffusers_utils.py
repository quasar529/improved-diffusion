"""
https://github.com/giannisdaras/ambient_utils/blob/main/ambient_utils/diffusers_utils.py
"""
    ...
    transform = transforms.Compose(
        [
            ...
            (
                ...
                else transforms.Resize(pil_image.size)
            ),
        ]
    )
...
def run_unet(pipe, noisy_latent, steps, resolution=None, captions=None, return_noise=False):
    ...
    add_time_ids = torch.cat(
        [compute_time_ids((resolution, resolution), (0, 0)) for _ in range(noisy_latent.shape[0])]
    )
...
def sample_with_early_stop(pipe, denoising_end, prompts, num_inference_steps=50, return_latent=False, **pipe_kwargs):
    ...
    discrete_timestep_cutoff = int(
        round(pipe.scheduler.config.num_train_timesteps - (denoising_end * pipe.scheduler.config.num_train_timesteps))
    )
    ...
    clean = run_unet(
        pipe, noisy_latents, torch.tensor([final_timestep], device=pipe.device), captions=prompts, return_noise=False
    )
    if return_latent:
        return clean
    else:
...
def load_model(
    input_dir, vae_path, sdxl_path, trained_with_lora=False, use_ema=False, device="cuda", weight_dtype=torch.float16
):
    ...
    if trained_with_lora or input_dir is None:
        ...
        if use_ema:
            unet = EMAModel.from_pretrained(
                os.path.join(input_dir, "unet_ema"), UNet2DConditionModel, torch_dtype=weight_dtype
            )
        else:
