"""
https://github.com/giannisdaras/ambient_utils/blob/2fe0e1388eba0fcb6680f86d7d43814cb13a2064/ambient_utils/__init__.py
"""
...
def from_x0_pred_to_xnature_pred_vp_to_vp(x0_pred, noisy_input, current_sigma, desired_sigma):
    ...
    return (
        (noise_coeff / desired_sigma) ** 2 * (ambient_sqrt(1 - desired_sigma**2) * x0_pred - noisy_input) + noisy_input
    ) / scaling_coeff


def from_x0_pred_to_xnature_pred_ve_to_ve(x0_pred, noisy_input, current_sigma, desired_sigma):
    current_sigma, desired_sigma = [broadcast_batch_tensor(x) for x in [current_sigma, desired_sigma]]
    return (current_sigma**2 - desired_sigma**2) / (current_sigma**2) * x0_pred + (
        desired_sigma**2 / current_sigma**2
    ) * noisy_input
...
def add_extra_noise_from_vp_to_vp(noisy_input, current_sigma, desired_sigma):
    ...
    scaling_coeff, noise_coeff, current_sigma, desired_sigma = [
        broadcast_batch_tensor(x) for x in [scaling_coeff, noise_coeff, current_sigma, desired_sigma]
    ]
    ...
    if save_wandb and wandb.run is not None:
        if wandb_down_factor is not None:
            ...
            pil_image = pil_image.resize(
                (pil_image.size[0] // wandb_down_factor, pil_image.size[1] // wandb_down_factor)
            )
...
def save_images(
    ...
    font_size=40,
    text_color=(255, 255, 255),
    draw_horizontal_arrow=False,
    draw_vertical_arrow=False,
):
    ...
    if save_wandb and wandb.run is not None:
        if wandb_down_factor is not None:
            ...
            grid_image = grid_image.resize(
                (grid_image.size[0] // wandb_down_factor, grid_image.size[1] // wandb_down_factor)
            )
...
def color_image_border(images, color_mask, border_thickness=5):
    ...
    colored_images[:, 0, :border_thickness, :] = (
        expanded_color_mask[:, 0, :border_thickness, :] * colored_images[:, 0, :border_thickness, :]
        + (1 - expanded_color_mask[:, 0, :border_thickness, :]) * 1
    )
    colored_images[:, 0, height - border_thickness - 1 :, :] = (
        expanded_color_mask[:, 0] * colored_images[:, 0, height - border_thickness - 1 :, :]
        + (1 - expanded_color_mask[:, 0]) * 1
    )
    colored_images[:, 0, :, :border_thickness] = (
        expanded_color_mask[:, 0, :, :border_thickness] * colored_images[:, 0, :, :border_thickness]
        + (1 - expanded_color_mask[:, 0, :, :border_thickness]) * 1
    )
    colored_images[:, 0, :, width - border_thickness - 1 :] = (
        expanded_color_mask[:, 0] * colored_images[:, 0, :, width - border_thickness - 1 :]
        + (1 - expanded_color_mask[:, 0]) * 1
    )
